...
function validateVersions(v) {
  ...
  const r = v.resources
  if (typeof r !== "object" || r === null) fail("resources: required object")
  if (!Number.isInteger(r.cpuCount) || r.cpuCount < 1) {
    fail(`resources.cpuCount: expected positive integer, got ${JSON.stringify(r.cpuCount)}`)
  }
  if (!Number.isInteger(r.memoryMb) || r.memoryMb < 512 || r.memoryMb % 2 !== 0) {
    fail(`resources.memoryMb: expected even integer >= 512, got ${JSON.stringify(r.memoryMb)}`)
  }
  const t = v.templates
...
const { cli, adapters, runtime, resources, templates, baseImage } = versions

// e2b `template create -d Dockerfile` builds the Dockerfile DIRECTLY and does
// NOT read this repo's e2b.template.toml (the CLI's own config file is
// `e2b.toml`; ours is a record only). Sandbox resources therefore cannot ride
// in the toml — they must be passed as CLI FLAGS at create time. The default
// is 512MB, which OOMs a heavy adapter install (mastra) at runtime. versions.json
// stays the single source; the generator threads these flags into every
// documented build command and records them in the toml.
const resourceFlags = `--cpu-count ${resources.cpuCount} --memory-mb ${resources.memoryMb}`
...
#   e2b template create ${templates.stable.alias} ${resourceFlags} -d Dockerfile
# Dev variant (same Dockerfile, different alias):
#   e2b template create ${templates.dev.alias} ${resourceFlags} -d Dockerfile
`
...
#   e2b template create ${templates.stable.alias} ${resourceFlags} -d Dockerfile   # see README.md
