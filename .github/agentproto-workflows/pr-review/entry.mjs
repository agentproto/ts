...
const reviewPrompt = (bindings) => {
  ...
  return [
    ...
    ``,
    `1. **POST THE REVIEW FIRST** (mandatory — the CI fails if you do not do this):`,
    `   You MUST call gh pr review before doing anything else. Even trivial PRs get a brief review.`,
    `   Run:`,
    `   \`\`\`bash`,
