...
Generic provider/backend **catalog**, **status**, **creds**, **setup-ledger**, **list/resolve**,
**MCP tool**, and **CLI wizard** primitives shared by the agentproto provider families
(agent-CLI, browser, tunnel, and eval-reporter backends).
...
importing zero concrete providers — everything flows through injected
...
The previous package name `@agentproto/adapter-kit` remains available as a deprecated shim that re-exports `@agentproto/provider-kit` unchanged.
