...
import { parseKnowledgeManifest } from "@agentproto/knowledge/manifest"
import { parseCollectionManifest } from "@agentproto/collection/manifest"
import { parsePlaybookManifest } from "@agentproto/playbook/manifest"
...
// Discovered via grep: only AIP-16 IO and AIP-17 RUNNER are cross-referenced today.
...
describe("M0 — marketing fixture workspace conformance", () => {
  describe("AIP-10 KNOWLEDGE (workspace + entries + source)", () => {
    ...
    it("source validates", () => {
      const fm = readFrontmatter("sources/fresh/tiktok-hook-2026-05.md")
      assertValid(validators.knowledge, fm, "tiktok-hook-2026-05.md (source)")
    })
    ...
    // generated by scaffold-aip; the zod manifest passes 3 of 3 oneOf branches
    // (entry, source, workspace), which throws "Passed 3" on real data.
    // The actual validation runs above against the JSON Schema. Re-enable when
    // zod is regenerated/hand-tuned from KNOWLEDGE.schema.json.
    it.skip("parseKnowledgeManifest accepts the workspace (blocked on zod regen)", () => {
      ...
      expect(() => parseKnowledgeManifest(src)).not.toThrow()
    })
  })
  ...
  describe("AIP-12 PLAYBOOK", () => {
    it("PLAYBOOK.md validates", () => {
      const fm = readFrontmatter("playbooks/landing-page-copy/PLAYBOOK.md")
      assertValid(validators.playbook, fm, "landing-page-copy/PLAYBOOK.md")
    })

    // KNOWN GAP — @agentproto/playbook/manifest zod is partially generated.
    // JSON Schema validation above is authoritative.
    it.skip("parsePlaybookManifest accepts it (blocked on zod regen)", () => {
      const src = readFileSync(
        path.join(FIXTURES_ROOT, "playbooks/landing-page-copy/PLAYBOOK.md"),
        "utf8",
      )
      expect(() => parsePlaybookManifest(src)).not.toThrow()
    })
  })

  describe("AIP-9 OPERATOR", () => {
    it("OPERATOR.md validates", () => {
      const fm = readFrontmatter("operators/marketing-analyst/OPERATOR.md")
      assertValid(validators.operator, fm, "marketing-analyst/OPERATOR.md")
    })

    it("parseOperatorManifest accepts it", () => {
      const src = readFileSync(
        path.join(FIXTURES_ROOT, "operators/marketing-analyst/OPERATOR.md"),
        "utf8",
      )
  ...
  describe("AIP-41 ROUTINE", () => {
    it("ROUTINE.md validates", () => {
      const fm = readFrontmatter("routines/daily-source-scout/ROUTINE.md")
      assertValid(validators.routine, fm, "daily-source-scout/ROUTINE.md")
    })

    it("parseRoutineManifest accepts it", () => {
      const src = readFileSync(
        path.join(FIXTURES_ROOT, "routines/daily-source-scout/ROUTINE.md"),
        "utf8",
      )
  ...
  describe("drift detection (intentional schema breakage must fail)", () => {
    ...
    it("rejects PLAYBOOK.md with bad evidence.kind enum", () => {
      const fm = readFrontmatter("playbooks/landing-page-copy/PLAYBOOK.md") as {
        evidence: Array<{ kind: string; ref: string }>
      }
      const tampered = {
        ...fm,
        evidence: [{ kind: "eval-case", ref: "anything" }], // valid enum: run|conversation|work-item|reflection|human
      }
      const ok = validators.playbook(tampered)
      expect(ok).toBe(false)
    })
  })
