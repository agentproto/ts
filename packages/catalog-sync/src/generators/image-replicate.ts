...
const ReplicateModelSchema = z
  .object({
    ...
    latest_version: z.object({
      id: z.string(),
      created_at: z.string(),
      openapi_schema: z.any(),
    }),
  })
  .passthrough()

const ReplicateSnapshotSchema = z
  .object({
    // DRF-paginated shape (live API)
    results: z.array(ReplicateModelSchema).optional(),
    next: z.string().nullable().optional(),
    previous: z.string().nullable().optional(),
    // Hand-authored fixture shape (committed snapshot)
    _meta: z.record(z.string(), z.unknown()).optional(),
    models: z.array(ReplicateModelSchema).optional(),
  })
  .passthrough()

type ReplicateSnapshot = z.infer<typeof ReplicateSnapshotSchema>
...
export const imageReplicate: CatalogGenerator = defineGenerator({
  ...
  async generate(ctx: GeneratorContext): Promise<GeneratedFiles> {
    ...
    const raw = await ctx.fetchSource(REPLICATE_SOURCE)
    const data = ReplicateSnapshotSchema.parse(raw)

    // Normalize: extract models from either `results` (live API) or `models` (fixture)
    const models = data.results ?? data.models
    if (!models) {
      throw new Error(
        "Replicate snapshot has neither `results` nor `models` array — invalid shape"
      )
    }

    // Filter to curated models only — live API returns unfiltered mix (text LLMs, video, etc.)
    const curatedModels = models.filter(m => MODEL_SPECS[m.name])
    if (curatedModels.length === 0) {
      const sampleNames = [...AGENT_VISIBLE].slice(0, 3).join(", ")
      throw new Error(
        `image:replicate: none of the curated models (${sampleNames}…) present in source payload (${models.length} entries) — API shape or roster drift`
      )
    }

    const entries = curatedModels.map(mapModel)
