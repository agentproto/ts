...
describe("sessionDisplayName (FIX D — the one name source of truth)", () => {
  ...
  it("ignores an already-persisted Claude local-command caveat title", () => {
    expect(sessionDisplayName(session({
      title: "<local-command-caveat>Caveat: The messages below were generated by the…",
      label: "claude-code",
      renamedByUser: false,
    }))).toBe("claude-code")
  })
