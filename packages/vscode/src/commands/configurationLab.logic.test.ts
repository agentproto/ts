...
describe("buildConfigurationLabSnapshot — opencode derived-from-model + modelDerivedApiKey", () => {
  it("scopes models by the derived route and makes the route runnable with an eligible profile", () => {
    const data = rawData({
      adapters: [
        adapter({
          slug: "opencode",
          name: "OpenCode",
          models: ["anthropic/claude-sonnet-4-5", "openai/gpt-5", "openrouter/anthropic/claude-sonnet-4-5"],
          modelDetails: [
            { id: "anthropic/claude-sonnet-4-5", provider: "anthropic" },
            { id: "openai/gpt-5", provider: "openai" },
            { id: "openrouter/anthropic/claude-sonnet-4-5", provider: "openrouter" },
          ],
          routeSelection: "derived-from-model",
        }),
      ],
      capabilities: [
        capabilities({
          adapter: "opencode",
          models: { defaultModel: "anthropic/claude-sonnet-4-5", supported: ["anthropic/claude-sonnet-4-5", "openai/gpt-5", "openrouter/anthropic/claude-sonnet-4-5"] },
          providers: [
            { id: "anthropic", ready: true },
            { id: "openai", ready: true },
            ...
            products: [
              {
                ...
                routes: [
                  ...
                  {
                    route: "openrouter",
                    ref: "anthropic/claude-sonnet-4-5@openrouter",
                    baseUrl: null,
                    pricing: null,
                    runnable: true,
                    eligibleProfiles: ["openrouter-api"],
      ...
      profiles: [
        profile({ id: "anthropic-api", endpoint: "anthropic", method: "api-key", label: "Anthropic API Key" }),
        profile({ id: "openai-api", endpoint: "openai", method: "api-key", label: "OpenAI API Key" }),
        profile({ id: "openrouter-api", endpoint: "openrouter", method: "api-key", label: "OpenRouter API Key" }),
      ],
    })

    // No explicit model: default to the adapter default (Anthropic) and resolve route/profile.
    const snapshot = buildConfigurationLabSnapshot(data, { adapter: "opencode" })
    expect(snapshot.selection.model).toBe("anthropic/claude-sonnet-4-5")
    expect(snapshot.selection.route).toBe("anthropic")
    expect(snapshot.selection.profile).toBe("anthropic-api")

    // Switching to an OpenAI model derives the openai route.
    const openaiSnapshot = buildConfigurationLabSnapshot(data, { adapter: "opencode", model: "openai/gpt-5" })
    expect(openaiSnapshot.selection.route).toBe("openai")
    expect(openaiSnapshot.selection.profile).toBe("openai-api")

    // OpenRouter model derives the openrouter route; no conflicting base_url is emitted.
    const openrouterSnapshot = buildConfigurationLabSnapshot(data, { adapter: "opencode", model: "openrouter/anthropic/claude-sonnet-4-5" })
    expect(openrouterSnapshot.selection.route).toBe("openrouter")
    expect(openrouterSnapshot.selection.profile).toBe("openrouter-api")
    const args = labSelectionToSpawnArgs(openrouterSnapshot)
    expect(args).toEqual({
      adapter: "opencode",
  ...
  it("blocks spawn when the selected route has no eligible auth profile", () => {
    const data = rawData({
      adapters: [
        adapter({
          slug: "opencode",
          ...
          modelDetails: [{ id: "anthropic/claude-sonnet-4-5", provider: "anthropic" }],
          routeSelection: "derived-from-model",
        }),
      ],
      capabilities: [
        capabilities({
          adapter: "opencode",
          models: { defaultModel: "anthropic/claude-sonnet-4-5", supported: ["anthropic/claude-sonnet-4-5"] },
          providers: [{ id: "anthropic", ready: false }],
        }),
      ],
      catalog: {
        vendors: [
    ...
    const snapshot = buildConfigurationLabSnapshot(data, { adapter: "opencode", model: "anthropic/claude-sonnet-4-5" })
    const error = snapshot.issues.find((i) => i.severity === "error" && i.axis === "route")
    expect(error?.message).toContain("no eligible auth profile")
  })
})
