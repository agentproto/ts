 ...
 * `@agentproto/apps` (the daemon-builtin session-story panel's house-app
 * source), reused byte-for-byte (the same validated surface the MCP-Apps
 * host renders). That html is fully self-contained (CSP inline-only)
...
import { SESSION_STORY_PANEL_HTML } from "@agentproto/apps/session-story"
