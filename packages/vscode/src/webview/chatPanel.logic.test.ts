...
import { buildChatPanelHtml, buildIframePanelHtml, chatPanelUrl, daemonOrigin } from "./chatPanel.logic.js"
...
describe("buildChatPanelHtml", () => {
  it("embeds the chat url in a full-viewport iframe", () => {
    ...
    expect(html).toContain(`<iframe id="app" title="Session Chat" src="${url.replace(/&/g, "&amp;")}"></iframe>`)
...
describe("buildIframePanelHtml", () => {
  it("uses the given title for both the <title> element and the iframe's title attribute", () => {
    const html = buildIframePanelHtml("http://h/apps/%40x%2Fmy-app/ui", "http://h", "My App")
    expect(html).toContain("<title>My App</title>")
    expect(html).toContain('<iframe id="app" title="My App" src="http://h/apps/%40x%2Fmy-app/ui"></iframe>')
  })

  it("escapes a title containing markup-significant characters in both spots", () => {
    const html = buildIframePanelHtml("http://h/ui", "http://h", `<script>&"</script>`)
    const escaped = "&lt;script&gt;&amp;&quot;&lt;/script&gt;"
  ...
  it("pins the CSP frame-src to the daemon origin with default-src 'none'", () => {
    const html = buildIframePanelHtml("http://127.0.0.1:18790/x", "http://10.0.0.5:18790", "My App")
    expect(html).toContain("default-src 'none'")
    expect(html).toContain("frame-src http://10.0.0.5:18790")
    expect(html).not.toContain("frame-src 'self'")
    expect(html).not.toContain("script-src")
  ...
  it("falls back to frame-src * when the daemon url doesn't parse (degraded but functional)", () => {
    const html = buildIframePanelHtml("http://127.0.0.1:18790/x", null, "My App")
    expect(html).toContain("frame-src *")
    expect(html).toContain("default-src 'none'")
  })
})
