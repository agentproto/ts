 ...
 *
 * Every DistillPort backend (the API-based AnthropicDistiller, the CLI-based
 * CliAgentDistiller) asks the same thing and reads the same shape back, so the
 * prompt and the tolerant JSON parse live here once. A backend only owns the
 * transport (HTTP vs. child process) and how it extracts the model's text.
 */

import { z } from "zod"
import { REFINED_KIND_SCHEMA, type DistillInput, type DistilledItem } from "@agentproto/corpus"
...
export function buildDistillPrompt(input: DistillInput, maxItems: number): string {
