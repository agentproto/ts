...
/**
 * Shared pagination primitives (moved here from `@agentproto/runtime`'s
 * `tool-envelope.ts` so the `paginated()` ToolTransformer can reuse the
 * exact cursor/limit semantics the daemon's list tools already depend on).
 * `@agentproto/runtime` re-exports this module verbatim, so existing
