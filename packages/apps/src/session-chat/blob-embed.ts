...
function transformChatHtmlForBlob(html, url, token){
  ...
  var shim = '<script>(function(){'
    ...
    + 'var _h = window.history, _rs = _h.replaceState.bind(_h), _ps = _h.pushState.bind(_h);'
    + 'function _sameOrLoc(u){'
    +   'try {'
    +     'var x = new URL(u == null ? window.location.href : u, document.baseURI);'
    +     'return x.origin === window.location.origin ? u : window.location.href;'
    +   '} catch (_) { return window.location.href; }'
    + '}'
    + '_h.replaceState = function(s, t, u){'
    +   'try { _rs(s, t, _sameOrLoc(u)); } catch (_) { try { _rs(s, t, window.location.href); } catch (_) {} }'
    + '};'
    + '_h.pushState = function(s, t, u){'
    +   'try { _ps(s, t, _sameOrLoc(u)); } catch (_) { try { _ps(s, t, window.location.href); } catch (_) {} }'
    + '};'
