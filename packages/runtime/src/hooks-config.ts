 ...
 * overrides the decision. `"hold"`/`"deny"` are additive engine substrate
 * for a later PR (the first real enforcing rule, `git push` → gate, ships
 * separately); this PR's own default config uses neither.
 *
 ...
 * rule whose action is `"hold"` or `"deny"` — that combination is a false
 * sense of safety: a Plane-1 hold/deny never fires in bypass posture, is
 * invisible to in-process tools, and doesn't exist for the 3 opaque
 * harnesses. Security intent must be expressed as `plane:"blast-radius"`
 * instead. `parseHooksConfig`/`validateHookRule` throw a `HooksConfigError`
 * for this at load time rather than silently under-enforcing it.
 */
...
export type HookAction = "log" | "allow" | "hold" | "deny"
export type HookDecision = "allow" | "hold" | "deny"
...
function validateHookRule(rule: unknown, index: number): HookRule {
  ...
  if (
    ...
    throw new HooksConfigError(
      `hooks.json rule "${id}" has invalid action ${JSON.stringify(r.action)} ` +
        `(expected "log" | "allow" | "hold" | "deny")`,
    )
  }
  const action = r.action

  // RISK-0 GUARD — see module docblock.
  if (intent === "security" && plane === "semantic" && (action === "hold" || action === "deny")) {
    throw new HooksConfigError(
      `hooks.json rule "${id}" declares intent:"security" with plane:"semantic" and action:${JSON.stringify(action)} — ` +
        `refusing to load. A Plane-1 (ACP permission-seam) hold/deny is bypassable: it never fires in bypass ` +
  ...
  return { id, plane, ...(intent ? { intent } : {}), match, action }
}
...
/**
 * Evaluate the rule table at the pre-exec (Plane-1) seam. The first
 * matching non-`"log"` rule wins; `"log"` rules tag a match for
 * observability but never change the decision (LOG-ONLY DEFAULT — see
 * module docblock). No match at all (including the empty rule set) falls
 * through to `fallback`.
 *
 ...
 * empty or log-only rule set reproduces today's behavior exactly.
 */
export function decide(
  ...
  for (const rule of rules) {
    ...
    if (matchesRule(rule.match, input)) return rule.action
  }
  return fallback
}
