/**
 * MCP protocol tests for the export_session tool.
 *
...
describe("export_session tool — registration", () => {
  it("is listed in tools/list with a description mentioning transcript", async () => {
    ...
    const tool = result.tools.find(t => t.name === "export_session")
...
describe("export_session tool — rendered content", () => {
...
describe("export_session tool — graceful error on unknown session", () => {
  ...
  it("does NOT throw — returns a structured isError result, never rejects", async () => {
    ...
    await expect(
      client.callTool({ name: "export_session", arguments: { sessionId: "sess_bad" } }),
