  ...
  const server: Server = createServer((req, res) => {
    void (async () => {
      try {
        ...
        // Routine routes — GET /routines returns AIP-41 routine DEFINITIONS
        // from the registrar (mirrors the MCP `routine_list` tool in
        // orchestration-tools.ts).
        if (opts.routineRegistrar && path === "/routines") {
          const handled = await handleRoutines(req, res, path, opts.routineRegistrar)
          if (handled) return
        }
 ...
 *
 * Answers from the AIP-41 registrar's `list()` — routine DEFINITIONS from
 * `.routines/*` (mirrors the MCP `routine_list` tool in
 * orchestration-tools.ts). Returns `true` when it handled the request so
 ...
 *
 * Mounted at a different prefix than /routines/* on purpose (see
 * routine-registrar.ts SPEC note).
 */
