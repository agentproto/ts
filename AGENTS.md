...
1. **Green gate.** Run this repo's own gate before calling anything done:
   ...
   **`pnpm test` is necessary but not sufficient — the CI gate is `pnpm test`
   ...
   step ("Type-check all packages", `ci.yml:182-186`, inside the
   build-and-test job); Vitest transpiles without type-checking, so a type error in a **test
...
comment; the switch logic is extracted to `scripts/agentflow/merge-gate.mjs`
...
- **CI-status gate.** Wait for a PR's checks as a shell gate:
  ...
  defaults to a 60s timeout (`gate.timeoutMs`, `supervisor.ts:281`) and caps
  out at 10 minutes; if your CI run is longer, set `timeoutMs` and lean on
  `onFail` retries as a poor-man's poll rather than expecting one attempt to
  cover a full run.
...
- **Risk fail-safe on merge.** `merge.alwaysEscalateGlobs` + the maintainer
  judge (`scripts/maintainer.mjs:109`, `buildJudgePrompt`) already escalate anything touching
  ...
  (`scripts/agentflow/merge-gate.mjs:49-51,85-92`, `SELF_MODIFICATION_RE`,
  with conformance tests in the merge-gate-test job, `ci.yml:443-471`).
...
- **Chaining, fan-in, and long-poll.** A policy's `next` field chains a
  ...
  `GET /policies/:id/wait` (`http-server.ts:2496-2518` dispatcher) is a
  blocking long-poll if you'd rather not spin a gate loop yourself.
