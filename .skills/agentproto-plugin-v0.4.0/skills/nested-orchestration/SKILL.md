...
(`agent_start`, `agent_prompt`, `session_monitor`,
`agent_output`, `session_tree`, `agent_kill`) — le parent ne
...
1. « Spawn N enfants EN PARALLÈLE (N appels `agent_start`), chacun avec sa
   tâche bornée passée via l'arg `prompt`. Donne à chacun un `label` distinct. »
2. « Fan-in : appelle `session_monitor({ sessionIds:[tous], event:"turn-end" })` et
...
2. « `session_monitor({ event:"awaiting-input" })` ; si timeout, lis la sortie pour
...
- **`session_monitor` rate les enfants ultra-rapides.** Un enfant trivial (haiku qui
  répond « 42 ») finit son tour en quelques secondes — parfois **avant** que le
  parent n'ait câblé son `session_monitor`. Le `turn-end` est un event transitoire :
  ...
  (a) le parent confirme via `agent_output` (le marqueur `turn-end
  (completed)` est dans le buffer) ; (b) prendre un curseur `session_events_poll({since})`
  **avant** de spawner et lire les events après. Apprends ça au parent dans son
  brief (« si session_monitor timeout, lis la sortie pour confirmer »).
...
- [ ] Brief du parent **nomme** ses outils d'orchestration + la parade session_monitor
