...
describe("LocalFs", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), "knowledge-files-"))
    fs = new LocalFs({ root })
  })
  ...
  it("writes then reads a file, creating parent dirs", async () => {
    await fs.writeFile("knowledge/notes/a.md", "hello")
    expect(await fs.exists("knowledge/notes/a.md")).toBe(true)
    expect(await fs.readFile("knowledge/notes/a.md")).toBe("hello")
  })

  it("does not leave the temp file behind after an atomic write", async () => {
    await fs.writeFile("knowledge/a.md", "x")
    const names = await fs.readdir("knowledge")
  ...
  it("walks recursively and returns workspace-relative posix paths", async () => {
    await fs.writeFile("knowledge/a.md", "a")
    await fs.writeFile("knowledge/sub/b.md", "b")
    const found = await fs.walk("knowledge")
    expect([...found].sort()).toEqual(["knowledge/a.md", "knowledge/sub/b.md"])
  })

  it("skips hidden segments while walking", async () => {
    await fs.writeFile("knowledge/visible.md", "v")
    await fs.writeFile("knowledge/.hidden/secret.md", "s")
    const found = await fs.walk("knowledge")
    expect([...found]).toEqual(["knowledge/visible.md"])
  })
