...
| `messaging`               | `{ allowSiblings?: boolean, agentInterrupt?: "allow" \| "deny" }` | Inter-session messaging policy ([`sessions` → Messages between sessions](../verbs/sessions.md#messages-between-sessions)). `allowSiblings` (default `false`) lets `message_send` / `message_reply` reach a sibling session. `agentInterrupt` (default `"deny"`) decides whether a **session** sender's `urgency: "interrupt"` may cancel the recipient's turn; when denied it's delivered as `steer` and the result reports it. Human (HTTP/CLI) senders always keep `interrupt`. |
