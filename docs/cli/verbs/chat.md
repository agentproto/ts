...
silently going nowhere. By default the route queues the prompt behind any in-flight turn rather
than rejecting it. `chat`'s own readline loop won't send a second prompt
while a turn is still in flight, but the underlying daemon route supports
`interrupt: true` (used by `agentproto sessions prompt --interrupt`) to
cancel the current turn and dispatch immediately.
