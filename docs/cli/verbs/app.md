...
| Flag | Default | Description |
|------|---------|-------------|
| `appDir` | current directory | Directory holding `.agentproto/APP.md` + `.agentproto/ui/`. Omit when serving an installed app by id with `--app`. |
...
| `--port <n>` | `PORT` env, then `ui.port` in `APP.md`, else OS-assigned | Port to bind. Resolution order: explicit `--port` > `PORT` env var > `APP.md` `ui.port` > OS-assigned. A declared `ui.port` that is already taken falls back to auto-assign; an explicit `--port` that is taken is a hard error. |
